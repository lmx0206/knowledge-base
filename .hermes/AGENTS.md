# Hermes Agent 配置 — mason 的 AI 知识库

> 这是 Hermes Agent 的配置文件
> Hermes 启动时会自动读取本文件
...
| 检查项 | 工具 | 命令 |
|--------|------|------|
| 格式检查 | markdownlint | `npm run lint` |
| 引用检查 | check-references.sh | `npm run check-refs` |
| 链接检查 | markdown-link-check | `npm run check-links` |
